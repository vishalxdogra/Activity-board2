...
import { Heart, MessageCircle, Users, MapPin, Calendar, Flag, CheckCircle } from 'lucide-react'
...
export default function ActivityCard({ activity, onLike, onJoin, onReport }: ActivityCardProps) {
          ...
          <div className="flex items-start gap-3 flex-1">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{activity.author.name}</span>
                ...
                <span className="text-xs text-gray-500 dark:text-gray-400">{activity.author.rollNumber}</span>
              </div>
            </div>
          </div>
          <div className="flex flex-col items-end gap-2">
            <span className={`px-3 py-1.5 rounded-full text-xs font-semibold shadow-sm ${
              activity.type === 'OPEN' ? 'bg-gradient-to-r from-green-400 to-emerald-500 text-white' :
              activity.type === 'COMMUNITY' ? 'bg-gradient-to-r from-blue-400 to-cyan-500 text-white' :
      ...
      <CardFooter className="pt-4 border-t border-gray-100 dark:border-slate-700">
        <div className="flex items-center justify-between w-full">
          <div className="flex items-center gap-6">
            ...
            >
              <Heart className={`h-5 w-5 ${liked ? 'fill-current animate-pulse' : ''}`} />
